...
- Shares successful Claude web fallback results through `~/.cache/pi-usage/claude-web.json`, with a 30-second fresh window and a cross-process lock. This file contains only parsed quota labels, percentages, reset times, and a cache timestamp—never cookies or raw responses. Stale windows are discarded after their reset time or after 24 hours.
