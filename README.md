...
- Refreshes asynchronously when a session starts, when the agent starts or settles, every 60 seconds while the agent is active, and whenever `/usage` is run.
...
- Keeps no cache and has no runtime dependencies. Its single polling timer exists only while the agent is active and is cleaned up when the agent settles or the session shuts down.
