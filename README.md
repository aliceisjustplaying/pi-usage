...
- Shares successful Claude web fallback results through `~/.cache/pi-usage/claude-web.json`, with a 30-second fresh window and a cross-process lock. This file is capped at 64 KiB and 32 quota windows and contains only parsed quota labels, percentages, reset times, and a cache timestamp—never cookies or raw responses. Windows are discarded at their reset time, reset-less windows after 5 minutes, and the entire cache after 24 hours.
